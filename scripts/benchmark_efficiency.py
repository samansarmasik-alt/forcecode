...
ForceCode verim=max (bu PR): 585 tok context + <20k output + idempotent retry
...
# CI'da forcecode.py tek dosya modül olarak yüklenir; paket kurulumu gerekmez (test_forcecode.py ile aynı yöntem)
_MODULE_PATH = pathlib.Path(__file__).resolve().parents[1] / "forcecode.py"
_SPEC = importlib.util.spec_from_file_location("forcecode", _MODULE_PATH)
assert _SPEC and _SPEC.loader
forcecode = importlib.util.module_from_spec(_SPEC)  # type: ignore
sys.modules.setdefault("forcecode", forcecode)
_SPEC.loader.exec_module(forcecode)  # type: ignore
...
claude_like_input = toks(forcecode.project_context(ROOT, "off", False))
# ForceCode verim=max pruned
forge_input = toks(forcecode.project_context(ROOT, "max", False, baseline=baseline, changed_only=changed))
...
out = {
  ...
  "forcecode_verim_max_context_tokens": forge_input,
  ...
  "forcecode_e2e_input_estimate": forge_e2e_input,
...
if not out["forge_efficient"]:
    print("BENCHMARK FAIL: ForceCode verim=max, Claude Code baseline'dan daha verimli değil")
    raise SystemExit(2)
